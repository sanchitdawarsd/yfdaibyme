...
contract Yfdai is Ownable {
    ...
    function buyToken(address _addr, uint256 amount) external payable {
        ...
        require(presale == false, "presale is over you cannot buy now");
         if(_addr == address(0) ){
            ...
            claimable[msg.sender] += msg.value.mul(updateRate()).mul(rate).div(1e18);
         }else{
             require(approvedAddresses[_addr] == true);
             if (_addr == addr1 || _addr == addr2) {
            claimable[msg.sender] += amount.mul(10**12).mul(rate).div(1e18);
          } else{ 
            claimable[msg.sender] += amount.mul(rate).div(1e18);
          }
         }
